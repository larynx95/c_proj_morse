...
typedef enum kind { ALPHA, NUM } Kind;
...
void encode_out(const char *s) {
  for (;; ++s) {
    ...
    if (isalpha(ch)) {
      ch = toupper(ch);
      fputs(table[ALPHA][ch - 'A'],
            stdout);  //`-'A'` depend on the sequence of character code
    } else if (isdigit(ch))
...
static void insert_aux(MTree **tree, char ch, const char *s) {
  if (*tree == NULL) *tree = calloc(1, sizeof(**tree));
  if (*s == '\0')
    (*tree)->value = ch;
  else if (*s == '.')
...
void make_tree(void) {
  ...
  // root->value = '/';//anything
  int i;
  for (i = 0; i < 26; ++i) insert('A' + i, table[ALPHA][i]);
  for (i = 0; i < 10; ++i) insert('0' + i, table[NUM][i]);
}
...
void drop_tree(void) { drop_tree_aux(root); }