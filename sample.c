...
int write_PCM16_stereo_header(FILE *file_p, int32_t SampleRate,
                              int32_t FrameCount) {
  int ret;
...
int generate_dual_sawtooth(double frequency1, double amplitude1,
                           double frequency2, double amplitude2,
...
int main(void) {
  ...
  /*Fill the buffer*/
  ret = generate_dual_sawtooth(frequency1, amplitude1, frequency2, amplitude2,
                               SAMPLE_RATE, FrameCount, buffer_p);
  if (ret < 0) {
  ...
  ret = write_PCM16_stereo_header(file_p, SAMPLE_RATE, FrameCount);
  if (ret < 0) {
  ...
  written = write_PCM16wav_data(file_p, FrameCount, buffer_p);
